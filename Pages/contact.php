...
<input type="text" id="name" name="name" value="<?php if (isset($_POST['name'])) {echo $_POST['name'];} elseif (isset($_SESSION['name'])) {echo $_SESSION['name'];}?>">
<span class="error">* <?php echo $namErr;?></span></div>
<div><label for="email">E-mail:</label><input type="email" id="email" name="email" value="<?php if (isset($_POST['email'])) {echo $_POST['email'];} elseif (isset($_SESSION['email'])) {echo $_SESSION['email'];}?>">
...
<div><textarea id="mess" name="mess" rows="8" cols="50" placeholder= "Tell us why you want to contact us!"><?php if (isset($_POST['mess'])) {echo $_POST['mess'];};?></textarea>
